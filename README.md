...
- **A verified-release deploy was interrupted between SSH steps:** verify that no `ops/deploy-release.sh` process still targets the VPS. Inspect `/opt/shelter/releases/.deploy-release.lock` and its token-owned directory below `.incoming`; remove only that matching stale stage plus the lock's `pid`, `owner`, and `kind` files. Never replace or delete an existing version directory to retry a tag.
